...
- 🔄 支持热更新，提升开发体验
- 🔧 配备一键替换包名脚本，可一键替换 @AgentMarkdown，更名为属于自己的包名
...
"dev:docs": "pnpm -F @AgentMarkdown/docs run dev", // 启动文档应用
"dev:play": "pnpm -F @AgentMarkdown/playground run dev", // 启动演练场
"build": "turbo run build", // 构建所有包
"build:docs": "pnpm -F @AgentMarkdown/docs run build", // 构建文档应用
...
"rename-pkg": "bash ./scripts/rename-package.sh" // 一键重命名包，如：@AgentMarkdown -> @vue3-lib
```

## 🚀 快速开始

```bash
# 以下请替换为自己的包名，@AgentMarkdown 可以通过 rename-pkg 命令，一键更改为属于自己的包名，如 pnpm rename-pkg "@AgentMarkdown" "@vue3-lib"
